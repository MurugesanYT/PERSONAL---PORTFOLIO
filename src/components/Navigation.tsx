...
const Navigation = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      isScrolled 
        ? 'bg-gray-900/90 backdrop-blur-xl shadow-2xl border-b border-purple-500/20' 
        : 'bg-transparent'
    }`}>
            ...
            <div className="flex items-center space-x-1 bg-gray-800/50 backdrop-blur-md rounded-2xl px-6 py-3 border border-gray-700/50">
              {navItems.map((item, index) => (
                <a
                  key={item.href}
                  href={item.href}
                  className="relative group px-4 py-2 text-sm font-medium text-gray-300 hover:text-white transition-all duration-300 rounded-xl"
                >
            ...
            <div className="px-6 py-4 space-y-2">
              {navItems.map((item, index) => (
                <a
                  key={item.href}
                  href={item.href}
                  className="group flex items-center space-x-3 px-4 py-3 text-gray-300 hover:text-white rounded-xl transition-all duration-300 hover:bg-gradient-to-r hover:from-purple-600/20 hover:to-blue-600/20 transform hover:translate-x-2"
